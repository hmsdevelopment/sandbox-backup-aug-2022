function processEmailNotifications()
{
	...
	nlapiLogExecution('DEBUG', 'Hour value', 'Hour = ' + hour);
	if(hour >= 18)
	{
		return;
	}
	...
	else
	{
		...
		for (var i=0; results != null && results.length > i; i++)
		{
			...
			nlapiLogExecution('DEBUG', 'Variables', 'Hours until showing = ' + hoursUntilShowing);
			...
			if(hoursUntilShowing > 24 && minutesSinceLastEmail > 30 && emailNotificationAttempts < 3)
			{
				nlapiLogExecution('DEBUG', 'Variables', 'Entered main if statement for record ' + id);
				...
				if(notificationMethod == '1' && enableEmailNotification == 'T')
				{
					if(statusID == '1' || statusID == '10' || statusID == '2' || statusID == '3') 
					{
						/*if(showingAssist == 'T')
						{
							...
							//nlapiSendEmail('3847', 'mhulshult@gmail.com', subject, body.getValue(), null, null, records);
						}
						
						else
						{*/
							//var body = nlapiMergeRecord(22, 'supportcase', id);
							nlapiLogExecution('DEBUG', 'Variables', 'Preparing to send email for record ' + id);
