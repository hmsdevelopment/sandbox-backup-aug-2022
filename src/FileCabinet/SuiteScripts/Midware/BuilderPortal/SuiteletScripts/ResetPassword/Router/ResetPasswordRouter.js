 ...
 */
define(["require", "exports", "N/log", "N/error", "N/http", "../Controllers/ResetPasswordController"], function (require, exports, log, error, http, controller) {
    ...
    function handleGet(pContext) {
        pContext.response.write({ output: controller.getResetPassword(pContext.request.parameters) });
    }
    function handlePost(pContext) {
        log.debug("handlePost", "nothing for the moment");
        pContext.response.write({ output: controller.processResetPassword(pContext.request.parameters) });
    }
