...
*/
define(["require", "exports", "N/file", "N/https", "N/log", "N/query", "N/record", "N/render", "N/runtime", "N/ui/serverWidget", "N/url"], function (require, exports, file, https, log, query, record, render, runtime, serverWidget, url) {
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.onRequest = void 0;
    let datatablesEnabled = true;
    let page;
    let remoteLibraryEnabled = true;
    let rowsReturnedDefault = 10;
    let scriptURL;
    let queryFolderID = null;
    let toolLatestVersion;
    let toolUpgradesEnabled = true;
    let toolUpgradeMessage = '';
    let version = '2021.2 Beta 2';
    let workbooksEnabled = true;
    function onRequest(pContext) {
        ...
        if (pContext.request.method === 'POST') {
            var requestPayload = JSON.parse(pContext.request.body);
            pContext.response.setHeader({
                name: 'Content-Type',
                value: 'application/json'
            });
            switch (requestPayload['function']) {
                case 'documentSubmit':
                    return documentSubmit(pContext, requestPayload);
                    break;
                case 'queryExecute':
                    return queryExecute(pContext, requestPayload);
                    break;
                case 'sqlFileExists':
                    return sqlFileExists(pContext, requestPayload);
                    break;
                case 'sqlFileLoad':
                    return sqlFileLoad(pContext, requestPayload);
                    break;
                case 'sqlFileSave':
                    return sqlFileSave(pContext, requestPayload);
                    break;
                case 'localLibraryFilesGet':
                    return localLibraryFilesGet(pContext);
                    break;
                case 'workbookLoad':
                    return workbookLoad(pContext, requestPayload);
                    break;
                case 'workbooksGet':
                    return workbooksGet(pContext);
                    break;
                default:
            ...
            if (pContext.request.parameters.hasOwnProperty('function')) {
                if (pContext.request.parameters['function'] == 'documentGenerate') {
                    documentGenerate(pContext);
                }
                if (pContext.request.parameters['function'] == 'upgrade') {
                    let upgradeSuccess = toolUpgrade(pContext);
                    if (upgradeSuccess === true) {
                        pContext.response.write(`<meta http-equiv="refresh" content = "0; url = ${scriptURL}" />`);
                    }
                    else {
                        pContext.response.write('<p>An error occurred during the upgrade process.</p><p>See the script execution log for details.</p><p><a href="javascript:history.go(-1);">Continue...</a></p>');
                    }
                }
            }
            else {
                ...
                pContext.response.writePage(form);
            }
        }
    }
    exports.onRequest = onRequest;
    function documentGenerate(pContext) {
        try {
            ...
            if (docInfo.docType == 'pdf') {
                ...
                pContext.response.write(pdfString);
            }
            else {
                ...
                pContext.response.write(htmlString);
            }
        }
        catch (e) {
            log.error({ title: 'documentGenerate Error', details: e });
            pContext.response.write('Error: ' + e);
        }
    }
    function documentSubmit(pContext, requestPayload) {
        try {
            var responsePayload;
            var sessionScope = runtime.getCurrentSession();
        ...
        pContext.response.write(JSON.stringify(responsePayload, null, 5));
    }
    ...
    function localLibraryFilesGet(pContext) {
        var responsePayload;
        var sql = `
		SELECT
			ID,
    ...
    function queryExecute(pContext, requestPayload) {
        try {
            var responsePayload;
            var moreRecords = true;
            var records = new Array();
            ...
            if (records.length > 0) {
                ...
                var queryResults = query.runSuiteQL({ query: paginatedSQL, params: queryParams }).asMappedResults();
                totalRecordCount == queryResults[0].totalrecordcount;
            }
    ...
    function sqlFileExists(pContext, requestPayload) {
        var responsePayload;
        var sql = `
		SELECT
			ID
    ...
    function sqlFileLoad(pContext, requestPayload) {
        var responsePayload;
        try {
    ...
    function sqlFileSave(pContext, requestPayload) {
        var responsePayload;
        try {
    ...
    function workbookLoad(pContext, requestPayload) {
        var responsePayload;
        try {
    ...
    function workbooksGet(pContext) {
        var responsePayload;
        var sql = `
		SELECT
			ScriptID,
