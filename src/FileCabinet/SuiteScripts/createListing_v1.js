...
nlapiLogExecution('DEBUG', 'Line no','12');
	  var date = new Date();
...
nlapiLogExecution('DEBUG', 'Line no','22');
	 ...
	 var custpage_builder_personnel = form.addField('custpage_builder_personnel', 'select', 'If someone else is in charge of building permits select them here and a request for a permit will be sent to them');
	  ...
	  nlapiLogExecution('DEBUG', 'Line no','52');
	  ...
	  {
        nlapiLogExecution('DEBUG', 'Line no','69');
		...
		//	var record = nlapiLoadRecord('customrecord_subdivision', recordID);
          ...
          var subdivisionName = nlapiLookupField('customrecord_subdivision',recordID,'custrecord_subdivision_id');//record.getFieldValue('custrecord_subdivision_id');
			subdivisionField.addSelectOption(recordID, subdivisionName);
			var mlsRegion1 = nlapiLookupField('customrecord_subdivision',recordID,'custrecord_mls_region_1');//record.getFieldValue('custrecord_mls_region_1');
			var mlsRegion2 = nlapiLookupField('customrecord_subdivision',recordID,'custrecord_mls_region_2');//record.getFieldValue('custrecord_mls_region_2');
			if(mlsRegion1 != null && mlsRegion1 != '' && setPermitField == 'F')
			{
//				var mlsRegionRecord = nlapiLoadRecord('location', mlsRegion1);
				var permitRequired =nlapiLookupField('location',mlsRegion1,'custrecord_permit_required');// mlsRegionRecord.getFieldValue('custrecord_permit_required');
				if(permitRequired == 'T')
				{
					setPermitField = 'T';
				}
			}
			nlapiLogExecution('DEBUG', 'Line no','87'+mlsRegion2);
			if(mlsRegion2 != null && mlsRegion2!= '' && setPermitField == 'F')
			{
				//var mlsRegionRecord = nlapiLoadRecord('location', mlsRegion2);
				var permitRequired =nlapiLookupField('location',mlsRegion2,'custrecord_permit_required');// mlsRegionRecord.getFieldValue('custrecord_permit_required');
				if(permitRequired == 'T')
				{
					setPermitField = 'T';
				}
	  ...
	  nlapiLogExecution('DEBUG', 'Line no','99');
	  ...
	  nlapiLogExecution('DEBUG', 'Line no','170'+floorPlanResults.length);
     for(var n=0; floorPlanResults != null && floorPlanResults.length > n; n++)
	  {
		var floorPlanID = floorPlanResults[n].getId();
        
        var floorPlanName = nlapiLookupField('customrecord_floorplan', floorPlanID, 'name');
...
//		var floorPlanRecord = nlapiLoadRecord('customrecord_floorplan', floorPlanID);
		...
		floorplanField.addSelectOption(floorPlanID, floorPlanName);
	  	  ...
	  	  nlapiLogExecution('DEBUG', 'Line no','179');
	  ...
	  nlapiLogExecution('DEBUG', 'Line no','183');
      //Set the property
   ...
   else
   {
	...
	//Create the Floorplan, if entered
		  ...
		  if(photoValue != null && photoValue != '')
		  {
		   ...
		   var pthouseno = proprecord.getFieldValue('custrecord_house_number');
			...
			if(builderpersonnel)
			{
				...
				var pformurl = nlapiResolveURL('SUITELET','customscript_hms_show_data_property','customdeploy_hms_show_data_property',true)+'&pid='+propertyRecordID;
		  ...
		  var redirectURL = nlapiResolveURL('SUITELET','customscript98','customdeploy1',true)+"&partnerid=" + userID + "&builderid=" + builderDivisionID + "&propertyid=" + propertyRecordID;
		  response.sendRedirect('external',redirectURL);
