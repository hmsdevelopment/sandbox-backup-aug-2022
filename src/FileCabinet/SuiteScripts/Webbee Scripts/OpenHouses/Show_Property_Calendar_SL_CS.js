...
function StartProcess()
{
	try
	{
		...
		if(process == 1)
		OpenProperties(request, response);
		else
...
function GetHTML(builderId,builderPersId)
{
	...
	{	
		...
		nlapiLogExecution('DEBUG','builderId && personnel Id',builderId+'-'+builderPersId);
		var divList = '';
		var divList2 = '';
		var divList3  = '';
		var mlsArea;
		...
		if(defVal(builderId) != '' && builderId != 0 && builderId > 0) 	//builderId = 3675;
		{
			nlapiLogExecution('DEBUG','49','49');
			var mlsArea = nlapiLookupField('customer', builderId, 'custentity_mls_service_regions');
		...
		if(defVal(builderPersId) == ''){
			nlapiLogExecution('DEBUG','first filter ', builderId+'-'+builderPersId);
			filters.push(new nlobjSearchFilter('custrecord12', null, 'is', builderId));
		}
		else if(defVal(builderPersId) != ''){
			nlapiLogExecution('DEBUG','second filter ', builderId+'-'+builderPersId);
			filters.push(new nlobjSearchFilter('custrecord_property_bsr_team', null, 'is', builderPersId));
		}
		
		nlapiLogExecution('DEBUG','76','76');
	//	filters.push(new nlobjSearchFilter('custrecord_property_status', null, 'anyof', [1,8,11]));
		filters.push(new nlobjSearchFilter('custrecord_property_status', null, 'is', '1'));
		...
		columns.push(new nlobjSearchColumn('custrecord_subdivision_id','custrecordcustrecordsubdname'));	
		...
		var results = nlapiSearchRecord('customrecord_property_record', null, filters, columns);		
		...
		+'<li><a data-toggle="tab" href="#menu1">Last Week\'s Open Houses</a></li>'
		+'</ul>'
		...
		+'</div>'		
		+'<div class="col-sm-3">'
		+'<input type="hidden" id="builderid" value="'+builderId+'">'
		+'<input type="hidden" id="mlsareaid" value="'+mlsArea+'">'
		+'<h4 class="text-center">Use the slider below to set the start and end times.</h4>'
		...
		+'</div>'		
		+'<div class="col-sm-6">'
		+'<h4 class="text-center">Selected Open Houses (Click Properties and Dates to Remove Them)</h4>'
		...
		+'<button class="btn btn-info btn-block buton" id="rg-event" style="font-size:medium;"  onclick="createEvent1()">Add Above Properties (They can be modified before final submission)</button>'
		+'</div>'
		+'</div>'		
		+'</div>'
		+'<br><br>'
		...
		+'<div class="row">'
		+'<div class="col-sm-12">'
		+'<h3>Summary of Open Houses Selected</h3>'
		+'</div></div>'
		+'<div class="row">'
		+'<div class="col-sm-10">'
		+'<div class="event-box">'
		...
		//+'<div class="col-sm-2">'
		//+'<h3> </h3>'
		+'<button class="buton buton-green" id="rg-event" style="font-size:large;"  onclick="registerEvent()">Submit These Open Houses to HMS</button>'
		+'<br />'
		+'</div>'
		+'</div>'
		+'<!-- Modal -->'
...
function GetFutureProperties(builderId)
{
	try
	{
		...
		filters.push(new nlobjSearchFilter('custrecord_open_date', null, 'after', date));
		...
		var results = nlapiSearchRecord('customrecord_open_house_properties', null,filters,columns);	
		if(results != null && results.length > 0)
		{
			len = results.length;	
			if(len > 10)
			...
			for(var i=0; i<len ;i++)
			{
				...
				var propName = nlapiLookupField('customrecord_property_record',key,'custrecord31',true);
				var data2 = nlapiLookupField('customrecord_property_record',key, ['custrecord_house_number','custrecord_lot_number']);				
				var value = defVal(data2.custrecord_house_number)+" "+defVal(propName)+" "+defVal(data2.custrecord_lot_number);

				var evhtml='';
				evhtml +='<div class="evbox" time="'+openTime+'" date="'+openDate+'" add="'+key+'">';
...
function GetPreviousWeekProperties(builderId)
{
  try
  {
		...
		filters.push(new nlobjSearchFilter('isinactive', null, 'is', 'F'));
		filters.push(new nlobjSearchFilter('custrecord_builder', null, 'is', builderId));
		filters.push(new nlobjSearchFilter('custrecord_open_date', null, 'within', newDate, date));
		...
		if(results != null && results.length > 0)
		{
			...
			for(var i=0; i<len ;i++)
			{
				...
				var openTime = defVal(results[i].getValue(columns[2]));
				var data = nlapiLookupField('customrecord_property_record',key,['custrecord_property_status','custrecord_house_number','custrecord31','custrecord_lot_number'],true);
				var data2 = nlapiLookupField('customrecord_property_record',key, ['custrecord_house_number','custrecord_lot_number']);				
				var status = data.custrecord_property_status;
				var value = defVal(data2.custrecord_house_number)+" "+defVal(data.custrecord31)+" "+defVal(data2.custrecord_lot_number);

				if(status == 'Available')
				{
					divList += '<div class="bg-primary n-data2" key="'+key+'" openDate="'+openDate+'" nextDate="'+nextDate+'" openTime="'+openTime+'" data-toggle="tooltip" data-placement="right" title="Time:'+openTime+' Date:'+openDate+'">'+value;
					divList += '</div>';
				}
				if(status == 'Pending')
				{
					divList += '<div class="bg-gray"  disabled key="'+key+'" openDate="'+openDate+'" nextDate="'+nextDate+'" openTime="'+openTime+'">'+value;
					divList += '</div>';	
				}
			}
		}			
...
function AddOpenHouse(builderId,mlsAreaId,propertyId,openDate,openTime)
{
	...
	{
		var filters = [];
		filters.push(new nlobjSearchFilter('custrecord_open_property', null, 'is', propertyId));
		filters.push(new nlobjSearchFilter('custrecord_builder', null, 'is', builderId));
		filters.push(new nlobjSearchFilter('custrecord_mls_region', null, 'is', mlsAreaId));
		filters.push(new nlobjSearchFilter('custrecord_open_date', null, 'on', openDate));
		filters.push(new nlobjSearchFilter('custrecord_open_time', null, 'is', openTime));
		...
		if(results == null)
		{
			...
			var propUrl = nlapiResolveURL('RECORD', 'customrecord_open_house_properties', recId);
			propUrl = 'https://1309901.app.netsuite.com/'+propUrl;
