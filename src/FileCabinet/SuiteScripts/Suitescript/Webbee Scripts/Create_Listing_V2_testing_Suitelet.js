	...
	try {
		...
		if (request.getMethod() == 'GET') {
			...
			if (propertyID != null && propertyID != '' && propertyID != undefined) {
				...
				var file_Load;
				try {
					file_Load = nlapiLoadFile(file_id);
				} catch (error) {
					nlapiLogExecution("ERROR", "File not found", "Market Home Information Sheet file not found for subdivision " + subdivisionvalue);
				}

				var urlFile = file_Load ? file_Load.getURL() : '';
				...
				nlapiLogExecution("DEBUG", "customrecord_street_name loaded", streetValue);
          ...
          // STREET TYPE
			...
			if (streetResults == null) {
                streetTypeSuffixOptions.push("No Street Suffix");
            } else {
           ...
           if (sResults == null) {
				...
				nlapiLogExecution("DEBUG","results sale status length", sResults.length);
		...
		html += '<script src="https://code.jquery.com/jquery-1.12.4.js"></script>' +
				...
				'</script>' +
				'<script>jQuery(document).ready(function() {' +
				"var input = document.getElementById('subdivisionval1');"+
				"getval(input);"+
				'});' +
				'</script>' +
				'<script> '+
				...
				'			jQuery("#tb").prop( "disabled", true ); '+
				'			jQuery("#tb").val(""); '+
				'			jQuery("#tb").val("HMS To Compose Marketing Remarks");'+
                '		} else{ '+
				'			jQuery("#tb").prop( "disabled", false ); '+
				'			jQuery("#tb").val(""); '+
				'		}'+
				'	});'+
			    '});' +
			...
			html += '							</select>' +
				...
				'				     </div> 	' +
				'					 <div class="col-sm-6">' +
				'					      <label id = "street_type_label" >Street Suffix</label>' +
				...
				'					<input onchange="this.value = Math.floor(Number(this.value))" onkeypress="return event.charCode >= 48 && event.charCode <= 57" type=\'number\' name=\'lastprice\' placeholder=\'List Price\'value=\'' + (listPriceFieldValue || "") + '\'  id=\'listingprice\' pattern=\'\\d*.\d{1,2}\'> ' +
				'				     </div> 	' +
				'' +
				'' +
				'					 <div class="col-sm-6">' +
...
//				'				   </div>' +
			...
			*/
			...
			html += '							</select>	 ' +
				...
				'    								<label class="custom-file-label" for="information" id="custom-file-label-information" >Choose File</label> ' +
                '				             </div> 		' +
                '				        </div> 		' +
                '				     </div> 		' +
                '				   <div class="raw mb20">				' +
                ...
                '    							<label class="custom-file-label" for="selectionSheet" id="custom-file-label-selectionSheet" >Choose File</label> ' +
                '				         	 </div> 		' +
                '				        </div> 		' +
                '				     </div> 		' +
                '				   <div class="raw mb20">				' +
			...
			var formData = request.getAllParameters()
			 ...
			 try {
				...
				nlapiLogExecution("DEBUG", "ERROR", JSON.stringify(error));
			 }
			...
			for (var i = 0; i < fileNames.length ; i++){
				try {
					...
					requestFile.setDescription(params['description_file_'+fileNumber]);
					requestFile.setFolder(36830);
					...
					nlapiLogExecution("DEBUG", "FILE TEST ERROR", JSON.stringify(error));
					continue;
				}
			}
		...
		nlapiLogExecution("DEBUG", "Catch-error stack", JSON.stringify(err));
	}
...
function insertMultipleFilesDemoHTML(){
    return  '<script>'       +
    'var fileCount = 0;' +
    'function addFileContentToForm(input, i){ ' +
    '    var fileContentsInput = jQuery(\`<input name="custpage_file_\${fileCount}_contents" id="custpage_file_\${fileCount}_contents" type="text">\`); ' +
    ...
    '        var newHtml = \`<div class="raw mb20"> <div class="col-sm-12"> <label id="other_file_label" class="lg" style="display:inline-block;margin-top: 25px;"><span class="file-name">\${input.files[i].name}</span> succesfully added, please enter a description:&nbsp;<span style="color: #ff0000;display: inline-block;"> * </label><div class="custom-file" style="display:inline"> <input type="text" class="file-description" name="description_file_\${fileCount}" id="description_file_\${fileCount}" required><span><i class="fa fa-trash file-exclude" id="file-\${fileCount}-exclude"></i></span> </div> </div> </div>\`; '+
    ...
    '<div class="raw mb20" id="files-div"> ' +
    ...
    '        <div class="custom-file" style="display:inline">  ' +
    '            <input type="file" name="others" id="others"class="custom-file-input"  onchange="multipleFileInputChanged(event)" multiple>  ' +
    '            <label class="custom-file-labels" for="others" id="custom-file-label-others">Browse</label>  ' +
    '        </div> ' +
    '    </div> ' +
    '</div> ' +
    ' ' +
